...
void SetString(std::ofstream& s, std::string str)
{
    ...
    if (!IsAnsi(str))
    {
        ...
        size = -static_cast<int>(wstr.size());
...
std::ifstream& operator>>(std::ifstream& s, ModFile& m)
{
    ...
    const std::streamoff end = s.tellg();  // Use streamoff (signed) to avoid unsigned issues
    std::streamoff pos = end - sizeof(int);  // Use streamoff for positions
    s.seekg(pos);
    ...
    if (magic == PACKAGE_MAGIC)
    {
        ...
        m.Packages.resize(static_cast<size_t>(compositeCount));
        std::vector<int> offsets(static_cast<size_t>(compositeCount));
        ...
        for (int idx = 0; idx < compositeCount; ++idx)
        {
            ...
            if (idx)
            {
                m.Packages[idx - 1].Size = static_cast<int>(s.tellg()) - m.Packages[idx - 1].Offset;
            }
            s >> m.Packages[idx];
        }
        m.Packages.back().Size = (compositeEnd ? compositeEnd : static_cast<int>(end - metaSize)) - m.Packages.back().Offset;
    }
    else
    {
        ...
        p.Size = static_cast<int>(end);  // Explicit: streamoff to int
        s.seekg(0);
        s >> p;
    }
    return s;
...
std::ifstream& operator>>(std::ifstream& s, GameConfigFile& cfg)
{
    ...
    cfg.Mods.resize(static_cast<size_t>(count));
...
{
    int count = static_cast<int>(cfg.Mods.size());
...
{
    p.Offset = static_cast<int>(s.tellg());
    s.seekg(p.Offset + 4);
...
std::ifstream& operator>>(std::ifstream& s, ModFile::TfcPackage& p)
{
    ...
    p.IdxOffset = static_cast<int>(s.tellg());
